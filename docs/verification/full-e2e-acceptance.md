...
| AI intelligent-cover generation | FAILED (real provider) | Real disclosure modal showed exact Endpoint/model and supplemental prompt field; two confirmed generation attempts failed with the generic UI error, while an independent reproduction observed Agnes image HTTP 503. No candidate file or `cover` Frontmatter change was observed |
