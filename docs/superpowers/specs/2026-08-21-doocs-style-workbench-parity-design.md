...
- 工作台预览中的每个代码块提供键盘可达的一键复制按钮。按钮默认只显示小型复制图标，保持透明、无边框、无阴影；复制成功后原位切换为对钩，停留约 1 秒后自动恢复为复制图标；失败时切换为错误图标并更新可访问名称，不显示“复制中”“已复制”等状态文字。按钮只装饰预览 DOM，不写入 `RenderArtifact`，因此不会进入公众号富文本或草稿正文。
