...
- 请求体固定包含 `model`、`prompt`、`n: 1`。
- 首版不发送 `size`、`response_format`、`return_base64` 等可选或厂商扩展字段，由服务使用默认输出尺寸和格式，降低供应商兼容差异。
...
- 接受不同供应商默认输出尺寸不一致，由本地图片处理器统一裁剪为 2.35:1；首版不暴露厂商专有尺寸参数。
