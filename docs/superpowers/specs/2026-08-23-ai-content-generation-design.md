...
## 2026-08-26 已确认修订：Agnes / DeepSeek 独立供应商配置

本修订覆盖本文中与“完整 Endpoint、无厂商、无模型列表、固定 Agnes 图片请求”冲突的内容；其他候选、自动保存、封面采用和隐私规则继续有效。

### 结论与用户可见交互

- 当前设置页按用途展示供应商：`文本模型配置` 显示 `Agnes`、`DeepSeek`；`图片模型配置` 只显示已确认能生成图片的 `Agnes`。`OpenAI` 暂不展示、更不宣称已支持。
- 设置页先选择工作模式，而不是在顶部再做一次服务路由：`文本模型配置` 明确用于工作台的“生成标题”“生成摘要”，`图片模型配置` 明确用于“智能生成封面”。
- 每个模式内使用“左侧供应商列表 + 右侧供应商详情”布局。用户在右侧填写该模式、该供应商的 Base URL、API Key、请求格式并选择模型；点击唯一的“保存并设为当前文本模型”或“保存并设为当前图片模型”后，该模式的工作台请求只使用此配置。
- 文本与图片是两套独立配置、独立默认供应商和独立 SecretStorage Key。即使同一供应商也不得隐式复用 URL、模型或 Key。
- “获取模型列表”是用户显式点击的联网动作；仅在当前模式/供应商已经有可用 Base URL 和 Key 时请求。获取失败不能覆盖已保存模型，用户仍可手工录入模型名。
- Base URL 不要求用户填写完整 Endpoint。路径由明确的适配器按固定请求格式拼接；不提供完整 Endpoint 覆盖。

### 运行时契约

| 供应商 | 文本适配器 | 图片适配器 | 请求/响应边界 |
| --- | --- | --- | --- |
| Agnes | OpenAI Chat Completions | Agnes Images | 文本使用 `/chat/completions` 和 `choices[0].message.content`；图片保留已跑通的 Agnes 横向请求字段，接受 `data[0].url` 或 `data[0].b64_json`。 |
| DeepSeek | OpenAI Chat Completions | 不支持 | 文本使用 `/chat/completions` 和 `choices[0].message.content`。不向 DeepSeek 发送图片生成请求，也不在图片模型配置中展示 DeepSeek。 |

`deepseek-v4-flash-vision-exp` 是支持图片输入的多模态理解模型，不是图片生成模型。它可以出现在 DeepSeek 文本/多模态能力的模型发现结果中，但不得被保存、迁移或解析为智能封面的图片模型。

### 数据、密钥与迁移

`PluginSettings` 升级为 schema v5，持久化每种模式的供应商档案和当前供应商标识，不在 `data.json` 保存 API Key。SecretStorage Key 按 `模式 + 供应商` 区分，例如 `text/agnes` 与 `image/agnes` 为不同 Key。

旧 schema v4 的全局 `textApiEndpoint/textApiModel/textApiKey` 只在 URL 主机可明确识别为 Agnes 或 DeepSeek 时迁移到文本档案；图片配置只可迁移为 Agnes。历史 DeepSeek 图片档案必须取消激活且不可解析，已有密钥不删除也不再使用；无法识别的旧值保留为只读 legacy 数据并要求用户重新选择供应商，禁止暗中指定默认厂商或把 Key 发往新主机。

保存供应商档案不联网；模型发现、标题生成、摘要生成和封面生成均是用户触发的网络操作。切换供应商或覆盖 Endpoint 导致请求 origin 改变时，必须重新输入该档案对应的 Key。

### 验收

- 设置页不存在“顶部选择一次、下方再配置一次”的双重绑定；每种模式只有一个当前供应商及模型。
- 工作台“生成标题”“生成摘要”读取文本模式当前档案；“智能生成封面”读取图片模式当前档案。
- 验证 Agnes 的标题、摘要、封面三条用户路径，验证 DeepSeek 的标题、摘要两条用户路径；并验证图片模型配置中不显示 DeepSeek。每次实际请求在隔离 Vault 通过 SecretStorage 输入凭据，不将 Key、完整响应或图片 URL 写入仓库/日志/验证证据。
- 测试前构建并同步固定测试 Vault，关闭再重启 Obsidian 后走真实用户操作；失败以实际错误码、HTTP 状态和脱敏地址记录为 `BLOCKED` 或 `FAIL`，不得写成兼容。

### 2026-08-26 已确认修订：文本生成延迟与失败恢复

- 文本生成交互超时为 60 秒。该边界覆盖 Agnes 实机观测到的 23–35 秒响应抖动，但仍明显短于图片生成的 120 秒边界。
- 请求限制最大输出长度，避免供应商异常扩写。标题仍要求恰好三个合法且互不重复的候选；兼容 `titles` JSON 对象、三字符串 JSON 数组、三行纯文本和三行编号/项目符号列表。解释段落、错误数量、空值或重复结果仍整体失败。摘要优先读取 JSON 的 `digest` 字段，供应商仅返回非空纯文本时允许作为摘要候选，经清洗并截断到 120 字。畸形 JSON 仍按格式错误处理。
- 响应解析可从 Markdown 代码围栏、JSON 前后的无害说明文字及 OpenAI-compatible 文本分段中恢复 JSON；不得放宽标题数量、去重、长度及摘要长度校验。
- 超时、限流、鉴权/模型拒绝、网络错误和格式错误必须显示不同的安全提示，不得统一误报为“配置错误”。
- 文本生成不做隐式自动重试，避免重复计费；用户通过“重新生成”显式重试。

### 2026-08-26 已确认修订：固定协议与简化配置

- 每个已支持的供应商档案预填可修改的官方 Base URL：Agnes 文本和图片为 `https://apihub.agnes-ai.com/v1`，DeepSeek 文本为 `https://api.deepseek.com`。已有非空 Base URL 保持不变，插件不得在加载时覆盖用户值。
- Base URL 是唯一可编辑的服务地址字段；删除“完整 Endpoint（高级）”。标准历史 Endpoint 仅在其末尾符合固定路径时迁移为 Base URL；其他历史覆盖地址不再参与请求，用户需重新填写对应 Base URL。
- 请求格式不是用户配置项，而是“用途 + 供应商”的固定内置契约：Agnes/DeepSeek 文本为 OpenAI Chat Completions；Agnes 图片为 Agnes Images。设置页不展示该内部实现细节。
- API Key 仍仅持久化于 SecretStorage。设置页初始以密码框回显当前档案的已保存 Key；眼睛按钮可在本次打开的设置页中切换明文与隐藏。明文不写入 `data.json`、Frontmatter、日志、错误对象或验证证据。
- 验收必须覆盖：默认 Base URL、用户自定义 Base URL 保持、历史 Endpoint 迁移、固定格式不可被保存值篡改、文本与图片的 Key 初始隐藏/切换显示/切换回隐藏，以及 DeepSeek 不出现在图片模型配置。
...
| 封面生成 | 200 | 500 | 不发送 | 不发送 | 500 |

正文超限时采用“前 70% + 后 30%”截取，中间插入 `[内容已截断]`。这样保留开头主题和结尾结论，避免只发送开头。

封面生成是上述规则的例外：只发送当前文章标题、摘要和用户在当前弹窗填写的补充封面要求；不发送正文、标题层级或正文摘录。这样限制图片服务上下文、降低敏感内容外发范围，并保持封面主题由标题和摘要决定。
...
## 19. 工作台渲染生命周期（2026-08-26 修订）

- 工作台的“正在排版…”只能是短暂状态：一次渲染从快照读取到产物构建必须在 15 秒内完成；超过时限统一转入失败状态，不能永久占位。
- 渲染超时不复用旧文章产物，也不自动重试；用户修改笔记或重新打开工作台后才发起新的渲染，避免未知底层任务叠加。
- `ItemView` 关闭必须等待控制器停止、取消当前代际并释放订阅完成；下一次打开之后才能启动新的控制器代际。
- 对同一已启动控制器的重复 `start()` 调用必须重新请求当前笔记的渲染，保证 Obsidian 恢复视图后，重新创建的 DOM 不会保留空白或 loading 占位。
- 错误详情仅用于开发诊断；用户界面继续显示脱敏、可操作的“文章排版失败，请检查当前笔记”。

## 20. 设置页紧凑字段与密钥可见性（2026-08-26 修订）

- 公众号名称、AppID、AppSecret 使用同一紧凑字段行：标签与输入控件垂直居中，输入控件高度为 2.25rem；窄窗口改为单列，不能溢出。
- AppSecret 与 API Key 都从 SecretStorage 回显到当前设置 DOM，初始为 `password` 黑点隐藏；眼睛按钮只在当前设置页切换明文/隐藏，明文不写入任何普通设置、日志、报告或测试证据。
- 眼睛按钮必须位于输入框自己的定位容器内，不能相对于“标签 + 输入框”整行定位；文本与图片模型配置使用同一结构。
- 删除 AI 内容生成下的重复总说明；文本/图片页各自保留直接说明当前用途的单行文案。
- 三个公众号字段使用相同的输入列宽；AppSecret 控件占满该列并为无底色眼睛预留 3rem 内边距。模型 Key 使用相同无底色眼睛与内边距规则。
- 固定请求格式保留在内部适配器，不在配置页重复展示。

## 21. 临时模型下拉选择（2026-08-26 修订）

- “获取模型列表”成功后，不再渲染模型名称按钮或整行原生下拉框。模型输入框临时缩短，右侧仅展示一个带下箭头图标的紧凑按钮。
- 点击紧凑按钮才展开本次请求返回的模型列表；按钮应有可访问名称与展开状态。选择列表项会回写左侧模型输入框，并关闭展开列表，但紧凑箭头按钮继续保留。选择本身不保存配置、不触发额外联网；用户仍须点击“保存并设为当前模型”才使该模型生效。
- 紧凑箭头按钮必须始终与模型输入框同一行，包括设置弹窗的窄宽度；不得因响应式规则掉到下一行。按钮及其返回项目只在当前设置页、当前模式和当前供应商内存中存活。切换文本/图片模式或供应商时清空并恢复模型输入框整行宽度；再次点击获取模型列表后以新结果重建。
- 展开列表必须挂载到箭头按钮所属设置窗口的 `ownerDocument`，并位于设置卡片裁剪边界之外；禁止使用插件主窗口的全局 `document`，否则独立设置窗口只会更新按钮状态而看不到列表。浮层根据所属窗口的按钮位置与可视区域动态定位，设置可视最大高度和独立纵向滚动，不能被供应商卡片、设置弹窗或窗口底部截断。列表自身滚动时必须保持展开并允许继续选择下方模型；只有设置页外层滚动、切换模式或供应商、点击列表外部、调整所属窗口尺寸时才关闭浮层并释放监听。
- 获取中按钮禁用并显示反馈；失败、空列表时不显示下拉框，不能修改当前输入框或已保存模型。
- 下拉按钮和列表使用可访问名称、展开状态与列表语义，不能造成横向溢出。
- 紧凑箭头按钮不得写死独立高度；由模型输入框所在 Grid 行自动拉伸，保证上下边界与左侧输入框完全一致。当前模型的“保存并设为当前…”主按钮使用设置页现有微信绿主题变量，不能回落为 Obsidian 默认紫色；“获取模型列表”保持次要按钮样式。
